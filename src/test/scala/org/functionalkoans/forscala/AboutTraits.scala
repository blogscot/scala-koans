...
class AboutTraits extends KoanSuite {
  ...
  koan("Traits can have concrete implementations that can be mixed into concrete classes with its own state") {
    ...
    welder.logCache.size should be(1) // how many welds has welder made?
    baker.logCache.size should be(1)  // how many bakes has baker made?
  }
  ...
  koan("The diamond of death (http://en.wikipedia.org/wiki/Diamond_problem) is avoided since " +
    ...
    sb.mkString(";") should be("Creating C1;In T1: x=0;In T1: x=1;In T2: z=0;In T2: z=2;In T3: w=0;In T3: w=3;In C1: y=0;In C1: y=4;Created C1") // An elegant solution!
  }
}
